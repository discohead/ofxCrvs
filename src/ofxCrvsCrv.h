...
public:
  ...
  create(Box box, FloatOp op, std::shared_ptr<Crv> ampCrv,
         std::shared_ptr<Crv> rateCrv, std::shared_ptr<Crv> phaseCrv,
         std::shared_ptr<Crv> biasCrv, float ampOffset, float rateOffset,
         float phaseOffset, float biasOffset) {
    return std::make_shared<Crv>(box, op, ampCrv, rateCrv, phaseCrv, biasCrv,
                                 ampOffset, rateOffset, phaseOffset,
                                 biasOffset);
  }

  Crv() : Crv(FloatOp(), nullptr, nullptr, nullptr, nullptr) {}

  Crv(Box box)
      : Crv(box, FloatOp(), nullptr, nullptr, nullptr, nullptr, 1.0f, 1.0f,
            0.0f, 0.0f) {}
  ...
  Crv(FloatOp op, std::shared_ptr<Crv> ampCrv, std::shared_ptr<Crv> rateCrv,
      ...
      : Crv(Box(ofGetWidth(), ofGetHeight(), 0.f), op, ampCrv, rateCrv,
            phaseCrv, biasCrv, 1.0f, 1.0f, 0.0f, 0.0f) {}
  ...
  Crv(Box box, FloatOp op, std::shared_ptr<Crv> ampCrv,
      std::shared_ptr<Crv> rateCrv, std::shared_ptr<Crv> phaseCrv,
      std::shared_ptr<Crv> biasCrv, float ampOffset, float rateOffset,
      float phaseOffset, float biasOffset)
      : box(box), op(op), ampCrv(ampCrv), rateCrv(rateCrv), phaseCrv(phaseCrv),
        biasCrv(biasCrv), ampOffset(ampOffset), rateOffset(rateOffset),
        phaseOffset(phaseOffset), biasOffset(biasOffset) {
  ...
  std::vector<std::vector<float>> f2dArray(int numPoints, bool boxed,
                                           bool transformed,
                                           FloatOp samplingRateOp) const;
  std::vector<std::vector<float>>
  f3dArray(int numPoints, bool boxed, bool transformed,
           FloatOp samplingRateOp = FloatOp()) const;
